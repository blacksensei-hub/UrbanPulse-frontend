          ...
          >
            {col.links.map((l) => (
              <li key={l.to} className="pb-3">
                <Link to={l.to} className="text-muted hover:text-accent-text transition-colors">{l.label}</Link>
              </li>
            ))}
          </motion.ul>
        )}
  ...
  return (
    <footer className="above-harmattan relative mt-24 border-t border-border bg-surface">
      <div className="container-site">
        <MetaRow left="UrbanPulse / Accra, Ghana" right="05.79° N" />
      </div>
      <div className="container-site py-[var(--space-section)]">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
          ...
          <div className="hidden md:grid md:grid-cols-3 gap-8 lg:col-span-3">
            {COLUMNS.map((col) => (
              <div key={col.title}>
                <Label className="mb-4 block">{col.title}</Label>
                <ul className="flex flex-col gap-2.5">
                  {col.links.map((l) => (
                    <li key={l.to}>
                      <Link to={l.to} className="text-muted hover:text-accent-text transition-colors text-small">
                        {l.label}
                      </Link>
                    </li>
                  ))}
        ...
        <div className="mt-12 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-6">
          <Label>© {new Date().getFullYear()} UrbanPulse / Made in Ghana</Label>
          <div className="label-mono flex flex-wrap gap-5">
            ...
            <Link to="/returns-policy" className="hover:text-accent-text transition-colors">Returns</Link>
            <Link to="/shipping"       className="hover:text-accent-text transition-colors">Shipping</Link>
            <Link to="/contact"        className="hover:text-accent-text transition-colors">Contact</Link>
            <button type="button" onClick={openCustomize} className="hover:text-accent-text transition-colors">Cookie settings</button>
          </div>
        </div>
      </div>
    </footer>
